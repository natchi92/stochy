...
public:
  inputSpec_t() : stoch{true}, sigm{false}, par{false}, myModel(), myTask() {
    // shs_t<T, T2> model;
    // myModel = model;
    // taskSpec_t task;
    // myTask = task;
  ...
  inputSpec_t(bool stochastic, bool sigmoid, bool multi, shs_t<T, T2> model,
              taskSpec_t task)
      : stoch{stochastic}, sigm{sigmoid}, par{multi}, myModel(model),
        myTask(task) {}

  inputSpec_t(shs_t<T, T2> model, taskSpec_t task)
      : stoch{true}, sigm{false}, par{false}, myModel(model), myTask(task) {}
